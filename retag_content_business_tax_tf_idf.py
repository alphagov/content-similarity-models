...
def get_combined_text_for_taxon(content, taxon):
    ...
    return content[content['content_id'].isin(content_ids_for_taxon)]['combined_text']

def get_score_for_item(combined_text_for_content_item, sentences_for_taxon):
    sentences_for_taxon_list = sentences_for_taxon.to_list()
    if not sentences_for_taxon_list:
        return float("inf")
    vectorizer = TfidfVectorizer()
    print(sentences_for_taxon_list)
    vectorizer.fit(sentences_for_taxon_list)
    return vectorizer.transform([combined_text_for_content_item]).mean()
...
for index, row in problem_content.iterrows():
    ...
    if not current_taxon.all_siblings_and_children():
        ...
        combined_text_for_content_item = content[content['base_path'] == content_to_retag_base_path].iloc[0,:]['combined_text']
        all_combined_text_for_taxon = get_combined_text_for_taxon(content, current_taxon)
        # Get the score of the content item against the taxon it's currently in
        current_score = get_score_for_item(combined_text_for_content_item, all_combined_text_for_taxon)
        ...
        for taxon in current_taxon.all_siblings_and_children():
            sentences_for_taxon = get_combined_text_for_taxon(content, taxon)
            cosine_scores[taxon.title] = get_score_for_item(combined_text_for_content_item, sentences_for_taxon)
        # Sort the scores
        cosine_scores = sorted(cosine_scores.items(), key=operator.itemgetter(1))
        cosine_scores.reverse()
